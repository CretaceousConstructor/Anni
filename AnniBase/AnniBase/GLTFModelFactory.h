...
namespace Anni
{
	...
	public:
		GLTFModelFactory(
			...
			GLTFMetallicRoughness& mat_metallic_roughness_)
			: device_manager(device_manager_)
			, buf_fac(buf_fac_)
			, tex_fac(tex_fac_)
			, mat_metallic_roughness(mat_metallic_roughness_)
		{
		}
		...
		std::shared_ptr<LoadedGLTF> LoadGLTF(std::string_view filePath)
		{
			...
			//> load_2 init descriptor allocator
			...
			result_gltf.descriptor_allocator =
				std::make_unique<DescriptorAllocatorGrowable>(
					device_manager, gltf.materials.size(), sizes);
			//< load_2

			//> load_samplers
			for (fastgltf::Sampler& sampler : gltf.samplers)
			{
				VkSamplerCreateInfo sampl_CI = CI::PopulateTexSamplerCI();
			...
			result_gltf.materialDataBuffer = buf_fac.ProduceBufferReFac(
			...
			for (fastgltf::Material& mat : gltf.materials)
			{
				...
				result_gltf.materialDataBuffer->CopyFromHost(
					...
					mat_index * sizeof(GLTFMetallicRoughness::MaterialConstants));
				...
				// build material
				newMat->data = mat_metallic_roughness.WriteMaterial(passType, materialResources, *result_gltf.descriptor_allocator);
				mat_index++;
			}
			//< load_material
...
constexpr auto format_of_image =
  VK_FORMAT_R8G8B8A8_UNORM; // 难崩，怎么用UNORM读的
const auto img_view_CI =
  CI::PopulateTextureImgViewCI(format_of_image);
const auto tex_img_CI = CI::GetTextureImgCI(
  format_of_image, img_extend, Vk::ImgCINillFlag, path);
...
const auto img_view_CI =
  CI::PopulateTextureImgViewCI(format_of_image);
const auto tex_img_CI = CI::GetTextureImgCI(
  format_of_image, image_extend, Vk::ImgCINillFlag);
...
buffer.data);
			...
			std::span<const Vertex> vertices)
		{
			...
			buffer_4_mesh.vertexBuffer = buf_fac.ProduceBufferReFac(
				vertexBufferSize, CI::StorageBufferAddressable);
			VkBufferDeviceAddressInfo deviceAdressInfo{
			  .sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
			  .pNext = VK_NULL_HANDLE,
			  .buffer = buffer_4_mesh.vertexBuffer->GetGPUBuffer(),
			};
			buffer_4_mesh.vertexBufferAddress = vkGetBufferDeviceAddress(
				device_manager.GetLogicalDevice(), &deviceAdressInfo);
			buffer_4_mesh.indexBuffer =
				buf_fac.ProduceBufferReFac(indexBufferSize, CI::IndexBuffer);

			auto staging_buf = buf_fac.ProduceBufferReFac(
				vertexBufferSize + indexBufferSize, CI::StagingBuffer);
