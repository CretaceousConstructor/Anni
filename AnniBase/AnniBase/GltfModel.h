...
namespace Anni
{
	...
	public:
		void ProcessMaterial(const std::vector<VkDescriptorSetLayout>& set_layouts, const GFXPipelineCI& pipeline_para_pack, const VkPipelineBuilder& pipeline_builder, std::optional<std::vector<VkPushConstantRange>> push_constant_ranges_outside);
	...
	public:
		std::shared_ptr<Buffer> GetIdxBuf()
		{
			return indices.buffer;
		}
		std::shared_ptr<Buffer> GetVtxBuf()
		{
			return vertices.buffer;
		}

		std::vector<std::shared_ptr<Gltf::Image>> GetTexArray()
		{
			return images;
		}
	...
	template <typename M>
	VkGltfModel<M>::VkGltfModel(std::string model_path, std::vector<std::shared_ptr<Gltf::Image>>&& images_, std::vector<VkFormat>&& image_formats_, std::vector<Gltf::Texture>&& textures_, std::vector<std::shared_ptr<Material>>&& materials_, std::vector<Gltf::Node>&& nodes_, Gltf::VertexBufferGpu vertices_, Gltf::IndexBufferGpu indices_, Vk::ModelLoadingOption option_, GraphicsComponent& gfx_) :
	...
	template <typename M>
	void VkGltfModel<M>::ProcessMaterial(const std::vector<VkDescriptorSetLayout>& common_set_layouts, const GFXPipelineCI& pipeline_para_pack, const VkPipelineBuilder& pipeline_builder, std::optional<std::vector<VkPushConstantRange>> push_constant_ranges_outside)
	{
		//vkDestroyPipelineLayout(device_manager.GetLogicalDevice(), pipe_layout, nullptr);
		//vkDestroyDescriptorSetLayout(device_manager.GetLogicalDevice(), desc_layout, nullptr);
		...
		//****************************************************************************
		VkVertexInputBindingDescription binding_description0{};
		binding_description0.binding = 0;
		binding_description0.stride = sizeof(Gltf::Vertex);
		binding_description0.inputRate = VK_VERTEX_INPUT_RATE_VERTEX;

		static std::vector<VkVertexInputBindingDescription> VIBDS{ binding_description0 };
		...
		constexpr VkPushConstantRange    default_model_constant_range{
		...
		{
			push_constant_ranges.push_back(default_model_constant_range);
		}
	...
	void VkGltfModel<M>::DrawNode(VkCommandBuffer commandBuffer, const int node_index) const
	{
		...
		if (!node.mesh.primitives.empty())
		{
			...
			{
				if (primitive.idx_count > 0)
				{
					const auto& material = materials[primitive.material_idx];
					...
					vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, GetPipeline(primitive.material_idx));

					vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, GetPipelineLayout(), 1, 1, &GetDescSet(primitive.material_idx), 0, nullptr);

					vkCmdDrawIndexed(commandBuffer, primitive.idx_count, 1, primitive.fir_idx, 0, 0);
				}
			}
		}
