...
namespace Anni
{
	...
	ImageBaseReFac::ImageBaseReFac(GraphicsComponent& gfx_, vk::Image image_, ImageCIEnhanced& image_CI_) :
		gfx(gfx_), device_manager(gfx.DeviceMan()), image(image_), vk_image_CI(image_CI_)
	{
	}

	vk::Image ImageBaseReFac::GetImage() const
	{
		return image;
	}

	vk::Format ImageBaseReFac::GetImageFormat() const
	{
		return vk_image_CI.vk_image_CI.format;
	}

	const ImageCIEnhanced& ImageBaseReFac::GetCI() const
	{
		return vk_image_CI;
	}

	std::shared_ptr<TimelineSemWrapper>& ImageBaseReFac::GetSemUsedToTransfer()
	{
