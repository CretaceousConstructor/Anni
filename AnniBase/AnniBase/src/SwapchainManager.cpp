...
namespace Anni
{
	SwapchainManager::SwapchainManager(DeviceManager& device_manager_, WindowsSys& window_) :
	...
	void SwapchainManager::CreateSwapChainAndSwapImages()
	{
		...
		swapchain_CI.imageArrayLayers = 1;                                          //和VR有关，但是能不能用来:是不是msaa就不用resolve了,直接往上面渲就行了。没试过
		swapchain_CI.imageUsage = vk::ImageUsageFlagBits::eColorAttachment;        //直接往image渲染颜色
		swapchain_CI.imageSharingMode = vk::SharingMode::eExclusive;        //这里直接声明成exclusive，由我们自己来解决queue ownership transfer
		...
		swap_chain = device_manager.GetLogicalDevice().createSwapchainKHRUnique(swapchain_CI);
		//==========================================================================================================
		//BGRA SRGB
